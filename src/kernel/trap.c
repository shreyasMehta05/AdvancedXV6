...
/////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
void whenToYield(struct proc *process){
  if(process->execTime<process->slice) // if the process has not run for the complete time slice
  {
    return;
  }
  if(process->priorityLevel < _MAXLEVEL_MLFQ_ - 1) // if the process is not in the last queue 
  {
    process->priorityLevel++;
    process->slice = mlfqSlice[process->priorityLevel];
  }
  process->entryTime = ticks;
  process->execTime = 0;
  yield();
}
//////////////////////////////////////////////////////////////////////
...
void priorityBoost(struct proc *p){
  ...
  // printf("Priority Boost to pid:%d\n", p->pid);
  // time to boost the priority
  p->priorityLevel = 0;
  p->slice = _RESET_SLICE_MLFQ_;
  p->execTime = 0; // not sure about this
  return;
}
void incrementTicksBoost(){
  // printf("ticks boost: %d\n", __TICKS_BOOST__);
  if(__TICKS_BOOST__ >= _PRIORITY_BOOST_MLFQ_) 
  {
    __TICKS_BOOST__ = 0;
  } 
  ...
  //////////////////////////////////////////////////////////////////////
    ...
    for(int i=0;i<NPROC;i++){
      if(proc[i].state == RUNNABLE){
        acquire(&proc[i].lock);
        priorityBoost(&proc[i]);
        release(&proc[i].lock);
      }
    }
    //// GRAPH ////
    
    // write this to a file ../mlfq.txt
    
    /////

  #endif
  //////////////////////////////////////////////////////////////////////
  logicForTicking(p); // check if the alarm has run out
  //////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////
  #ifdef RR
